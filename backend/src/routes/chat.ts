...
router.post('/:patientId/:doctorId/message', async (req: Request, res: Response) => {
  try {
    ...
    if (!followRequest) {
      return res.status(403).json({ error: 'Follow request must be accepted before messaging' })
    }
...
router.get('/user/:userId', async (req: Request, res: Response) => {
  try {
    ...
    // Get chats where user is patient
    const patientChats = await Chat.find({ patientId: userId }).sort({ updatedAt: -1 })

    // Get chats where user is doctor
