#!/usr/bin/env python3
"""Melanoma image prediction helper used by the backend API."""
...
IMAGE_SIZE = (224, 224)
MODEL_PATH = Path(__file__).with_name("skincancer model") / "melanoma_classifier_model.keras"
...
def run_prediction(image_path: str):
    ...
    if error:
        if error.startswith("TensorFlow is not available"):
            ...
            is_melanoma = random.random() > 0.5
            confidence = round(random.uniform(0.7, 0.99), 4)
            return {
                "label": "Melanoma" if is_melanoma else "Not Melanoma",
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({"error": "No image path provided. Usage: python melanoma_predict.py <image_path>"}))
        sys.exit(1)
