...
export default function ChatPage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const userId = user?._id || user?.id || user?.userId;
      const otherUserId = user?.role === "patient" ? selectedChat.doctorId : selectedChat.patientId;
        ...
        <div className="flex-1 overflow-y-auto">
          {chats.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {chats.map((chat) => {
                const otherName = user.role === "patient" ? chat.doctorName : chat.patientName;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <p className="font-semibold text-gray-900 text-sm sm:text-base">{otherName}</p>
                      {lastMessage && (
                    ...
                    {lastMessage && (
                      <p className="text-sm text-gray-600 truncate">
                        {lastMessage.senderId === user._id || lastMessage.senderId === user.id || lastMessage.senderId === user.userId ? "You: " : ""}
                        {lastMessage.content}
                      </p>
                    )}
                  </button>
            ...
            <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-purple-600 shadow-sm flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">
                  {user.role === "patient" ? selectedChat.doctorName : selectedChat.patientName}
                </h2>
            ...
            >
              {messages.length === 0 ? (
                ...
                messages.map((msg, idx) => {
                  const isOwn = msg.senderId === user._id || msg.senderId === user.id || msg.senderId === user.userId;
                  return (
