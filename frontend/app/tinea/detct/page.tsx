...
// Model configuration
const MODEL_CONFIG = {
  detectEndpoint: "/api/detect/tinea",
...
export default function PsoriasisDetection() {
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      const isTinea = Boolean(data.is_tinea ?? String(data.label || "").toLowerCase().includes("tinea"));

      setPredictions(
        isTinea
          ? [
              { className: "Tinea Detected", probability: normalizedConfidence },
          ...
          : [
              ...
              { className: "Tinea Detected", probability: Math.max(0, 1 - normalizedConfidence) },
            ]
      );
    } catch (err) {
      const message =
